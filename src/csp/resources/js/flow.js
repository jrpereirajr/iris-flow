...
const handleExport = () => {
    ...
    postData('/csp/iris-flow/api/flow/save', diagram);

    console.log(flow);
    const data = postData('/csp/iris-flow/api/flow/generate', flow);
...
const handleOpen = async () => {
    ...
    try {
        const data = await getData(`/csp/iris-flow/api/flow/get/${prod_name}`);
        if (!!data.drawflow) {
            editor.import(data);
            toastr.success("", "Success!", {
                positionClass: "toast-bottom-right",
...
document.onreadystatechange = function (event) {
    if (document.readyState === "complete") {
        const getComponentOptions = () => {
            const response = fetch('/csp/iris-flow/api/components');
